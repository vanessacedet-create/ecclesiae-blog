      ...
      {featuredPost && (
        <section className="max-w-5xl mx-auto px-6 pt-12 animate-fade-up">
          <div className="ornamental-divider mb-8">
            <span className="font-display text-xs tracking-[0.3em] uppercase text-gold">Artigo em Destaque</span>
          </div>
          <PostCard id={featuredPost.id} title={featuredPost.title} date={featuredPost.date} excerpt={featuredPost.excerpt} category={featuredPost.category} featured={true} />
        </section>
      )}
      {restPosts.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 py-16">
          <div className="ornamental-divider mb-10">
            <span className="font-display text-xs tracking-[0.3em] uppercase text-gold">Artigos Recentes</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {restPosts.map((post, i) => (
              <div key={post.id} className={`animate-fade-up-delay-${Math.min(i + 1, 3)}`}>
                <PostCard id={post.id} title={post.title} date={post.date} excerpt={post.excerpt} category={post.category} />
              </div>
            ))}
          </div>
        </section>
